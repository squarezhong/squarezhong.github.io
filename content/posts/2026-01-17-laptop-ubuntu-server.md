...
3. 进入 BIOS，使用启动盘启动。部分联想电脑解锁隐藏 BIOS 可以参考 https://www.bilibili.com/opus/795391586576891909
4. 按照官方引导一步步安装系统即可，**LVM**(Logical Volume Management) 功能建议启用，方便后续分区调整。
...
# windows 使用如下命令
type $env:USERPROFILE\.ssh\id_ed25519.pub | ssh user@hostname.example.com "mkdir -p ~/.ssh && cat >> ~/.ssh/authorized_keys && chmod 700 ~/.ssh && chmod 600 ~/.ssh/authorized_keys"
```
3. 关闭 ssh 密码登录
```shell
...
![](Attachments/xiaoxin_fastfetch.png)
爽。
