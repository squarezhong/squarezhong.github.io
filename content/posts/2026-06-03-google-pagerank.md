...
基于上述假设，一个页面的 PageRank 值可以被递归地定义为 *所有链向它的页面的 PageRank 值除以各自出链数的总和*：
$$
...
**收敛性**：Google 矩阵 $\mathbf{G}$ 的第二大特征值满足 $|\lambda_2| \leq \alpha$ （参考论文 *The second eigenvalue of the Google matrix*），因此幂迭代以 $O(\alpha^k)$ 的速率收敛。当 $\alpha=0.85$ 时，约 $50\sim100$ 次迭代即可达到排序稳定。
