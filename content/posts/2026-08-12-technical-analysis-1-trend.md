...
---
技术分析系列文章会介绍一系列辅助技术分析的指标。
注意哪怕是同一个指标，不同网站不同软件的计算方式也有可能不同，建议使用前看看所用软件的文档。虽然文档里很大概率是一堆废话，写好几页也没把最关键的数学公式写出来。
...
$$
其中平滑系数 $\alpha = \displaystyle \frac{2}{N + 1}$
...
Smoothed Moving Average / Wilder's MA，平滑移动平均
$$
\text{SMMA}_N^{(t)}(X) = X^{(t)} \times \alpha + \text{SMMA}_N^{(t-1)}(X) \times (1 - \alpha)
$$
其中平滑系数 $\alpha = \displaystyle\frac{1}{N}$

SMMA 和 EMA 全部的区别就是平滑系数不同，数学上 $\text{SMMA}_N$ 与 $\text{EMA}_{2N-1}$ 等价，当然实际计算中会受到初值的影响，历史数据量足够大的时候初值的影响会基本归零。
